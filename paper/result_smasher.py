...
def extract_instance_name(filename):
    ...
    """
    match = re.search(r'RibeiroSuzarte(\d+).fjs', filename)
    if match:
        ...
        try:
            ...
            for run_number, row in enumerate(df.itertuples(index=False), start=1):
                ...
                all_results.append({
                    ...
                    'algorithm': algorithm,
                    'run': run_number,
                    'makespan': row.Makespan / 3600.0,  # Converter para horas
                    'time': row._1
                })
...
if __name__ == '__main__':
    ...
    input_directory = './paper/brute-data/RibeiroSuzarte'
    output_filepath = './paper/ribeirosuzarte.csv'
